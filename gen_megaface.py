...
def get_feature(imgs, nets, device):
    ...
    data = torch.zeros((count * 2, 3, 112, 112))  # imgs is list of PIL data
...
def main(config_file):
    ...
    facescrub_root=cfg.val+'/megaface/facescrub_images'
    megaface_root=cfg.val+'/megaface/megaface_images'

    facescrub_out = cfg.output+'/megaface/feature_out/facescrub'
    megaface_out  = cfg.output+'/megaface/feature_out/megaface'


    # model init
    nets = []
    image_shape = [int(x) for x in image_size.split(',')]
    for model in cfg.network.split('|'):  # args.model is the model path

        # net = sphere20a().to(device)
        resnet=get_model(model, dropout=0.0, fp16=cfg.fp16, num_features=cfg.embedding_size).cuda()
        # net_state = torch.load(model, map_location=device)
        weight = torch.load(cfg.output+"/model.pt")
    ...
    # facescrub_out = os.path.join(cfg.megaface_output, 'facescrub')
    # megaface_out = os.path.join(cfg.megaface_output, 'megaface')

    i = 0
    succ = 0
    buffer = []
    pbar = tqdm(open(facescrub_lst, 'r'))
    for line in pbar:  ##############
        if i % 1000 == 0:
        ...
        if img is None:
            ...
            continue
        out_path = os.path.join(out_dir, b +".bin")
        item = (img, out_path)
        buffer.append(item)
    ...
    succ = 0
    buffer = []
    pbar_mega = tqdm(open(megaface_lst, 'r'))
