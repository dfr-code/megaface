...
# devkit/templatelists/facescrub_features_list.json
...
def main():
    ...
    parser.add_argument('--config', default="megaface", help='the name of config file')#####insert

    parser.set_defaults(model=MODEL, num_sets=1, sizes=[10, 100, 1000, 10000, 100000, 1000000], probe_list='/media/sf_megaface-evaluation.pytorch/devkit/templatelists/facescrub_features_list.json', distractor_list_path=os.path.dirname(MEGAFACE_LIST_BASENAME))
    args = parser.parse_args()

    config = importlib.import_module("configs."+args.config)
    cfg = config.cfg()
    
    distractor_feature_path = os.path.join(cfg.output,'megaface/feature_out_clean/megaface')
    out_root = cfg.out_root
    probe_feature_path = os.path.join(cfg.output,'megaface/feature_out_clean/facescrub')
    model = args.model
    num_sets = args.num_sets
    sizes = args.sizes
    file_ending='.bin'#####################################改成没有，为了通用
