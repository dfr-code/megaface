...
def main(config_file):
    ...
    facescrub_noises=cfg.val+'/megaface/facescrub_noises.txt'
    megaface_noises=cfg.val+'/megaface/megaface_noises.txt'
    facescrub_lst=cfg.val+'/megaface/facescrub_lst'
    megaface_lst=cfg.val+'/megaface/megaface_lst'
    feature_dir_input= cfg.output+'/megaface/feature_out'
    feature_dir_output  = cfg.output+'/megaface/feature_out_clean'

    fs_noise_map = {}
    for line in open(facescrub_noises, 'r'):
        if line.startswith('#'):
            continue
        line = line.strip()
        fname = line.split('.')[0]
    ...
    fname2center = {}
    noises = []
    fs_bar = tqdm(open(facescrub_lst, 'r'))
    for line in fs_bar:
        if i % 1000 == 0:
            # print("reading fs", i)
        ...
        a, b = _path[-2], _path[-1]
        feature_path = os.path.join(feature_dir_input, 'facescrub', a, "%s.bin" % (b))
        feature_dir_out = os.path.join(feature_dir_output, 'facescrub', a)
        if not os.path.exists(feature_dir_out):
            os.makedirs(feature_dir_out)
        feature_path_out = os.path.join(feature_dir_out, "%s.bin" % (b))
        # print(b)
    ...
    for k in noises:
        ...
        f /= _norm
        feature_path_out = os.path.join(feature_dir_output, 'facescrub', a, "%s.bin" % (b))
        write_bin(feature_path_out, f)

    mf_noise_map = {}
    for line in open(megaface_noises, 'r'):
        if line.startswith('#'):
            continue
        line = line.strip()
        _vec = line.split("\t")
    ...
    i = 0
    nrof_noises = 0
    mf_bar = tqdm(open(megaface_lst, 'r'))
    for line in mf_bar:
        if i % 1000 == 0:
            # print("reading mf", i)
        ...
        feature_path = os.path.join(feature_dir_input, 'megaface', a1, a2, "%s.bin" % (b))
        feature_dir_out = os.path.join(feature_dir_output, 'megaface', a1, a2)
        if not os.path.exists(feature_dir_out):
            os.makedirs(feature_dir_out)
        feature_path_out = os.path.join(feature_dir_out, "%s.bin" % (b))
        bb = '/'.join([a1, a2, b])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', default="megaface", help='the name of config file')
